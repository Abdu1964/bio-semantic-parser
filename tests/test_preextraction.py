...
from src.preextraction.preextractor import Preextractor, _merge_entities
...
class TestNegationDetector:
    ...
    def detector(self):
        with (
            patch("src.preextraction.negation_detector.AutoTokenizer.from_pretrained") as mock_tok,
            patch("src.preextraction.negation_detector.AutoModelForSequenceClassification.from_pretrained") as mock_mdl,
        ):
...
class TestPreextractor:
    ...
    def preextractor(self):
        ...
        with (
            patch("src.preextraction.preextractor.spacy.load", side_effect=fake_spacy_load),
            patch("src.preextraction.negation_detector.AutoTokenizer.from_pretrained") as mock_tok,
            patch(
                "src.preextraction.negation_detector.AutoModelForSequenceClassification.from_pretrained"
            ) as mock_mdl,
        ):
            mock_tok.return_value = MagicMock()
            mock_mdl.return_value = MagicMock()
    ...
    def test_process_returns_dict_with_required_keys(self, preextractor):
        ...
        chunk = {
            ...
            "document_id": "doc-0000000001",  # >10 chars → stays offline (no PubTator3 call)
            "source_name": "pubmed",
            "section": "abstract",
            "chunk_index": 0,
            "total_chunks": 1,
    ...
    def test_process_sha256_doc_id_replaced_by_doi(self, preextractor):
        self._stub_negation(preextractor)
    ...
    def test_process_extracts_accession_numbers(self, preextractor):
        ...
        chunk = {
            ...
            "document_id": "doc-0000000002",  # >10 chars → stays offline (no PubTator3 call)
            "source_name": "pubmed",
            "section": "methods",
            "chunk_index": 0,
            "total_chunks": 1,
    ...
    def test_process_batch_returns_list(self, preextractor):
        self._stub_negation(preextractor)
        # >10 char doc_id -> no PubTator3 call
        chunks = [
            {"text": "Chunk one.", "document_id": "doc-batch-01", "source_name": "pubmed",
             "section": "abstract", "chunk_index": 0, "total_chunks": 2},
            {"text": "Chunk two.", "document_id": "doc-batch-01", "source_name": "pubmed",
    ...
    def test_process_pubmed_with_numeric_doc_id_calls_pubtator(self, preextractor):
        ...
        with patch("src.preextraction.preextractor.fetch_pubtator_entities", return_value=[]) as mock_pt:
            chunk = {
                "text": "Short text.",
