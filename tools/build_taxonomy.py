...
# Add project root so we can import our schema
...
# Maps alias predicate names from external schemas → our 57 canonical types.
# Used only by this tool to check taxonomy coverage; never called during extraction.
SYNONYM_MAP = {
...
# Direct mapping from OpenBioLink EdgeType enum name → our canonical type value.
# These compound names (GENE_DRUG, DRUG_INHIBITION_GENE, etc.) are not in
# SYNONYM_MAP (which uses verb-form aliases). We resolve them here in the script.
_OBL_CANONICAL = {
...
# Human-readable descriptions for each EdgeType enum member
# Derived from OpenBioLink paper (Breit et al. 2020) and source annotations
_OBL_DESCRIPTIONS = {
...
def _canonical_for(raw_name: str, source_schema: str = "") -> str:
    ...
    # OpenBioLink compound names resolved via direct map
    if source_schema == "openbiolink":
        return _OBL_CANONICAL.get(raw_name, "")

    # Biolink uses space-separated names ("associated with", "acts upstream of").
    # Normalise spaces and hyphens to underscores before synonym lookup.
    import re as _re
